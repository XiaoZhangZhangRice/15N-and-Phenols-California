...
```{r}
preplant_model <- lm(Yield~Field*Year, data=preplant)
anova(preplant_model)
pls205_diagnostics(preplant_model)
```

> Year effect significant.
> Model assumptions look satisfied.
...
```{r}
topdress_model <- lm(Yield~Field*Year, data=topdress)
anova(topdress_model)
pls205_diagnostics(topdress_model)
```
